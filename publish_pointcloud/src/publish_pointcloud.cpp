...
{  
	...
	ros::NodeHandle nh, np("~");  
        ...
        np.param<std::string>("path", path, "../data/test.pcd");
	np.param<std::string>("frame_id", frame_id, "camera");
	np.param<std::string>("topic", topic, "/pointcloud/output");
        np.param<int>("hz", hz, 5);
